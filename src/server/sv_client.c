...
void SV_DirectConnect( netadr_t from ) {
	...
	if ( version != PROTOCOL_VERSION && version != FACADE_PROTOCOL_VERSION ) {
...
void SV_SendClientGameState( client_t *client ) {
	...
	if( client->prevGamestateSent > (svs.time - 400) ) {
		return;
	}
	client->prevGamestateSent = svs.time;
...
void SV_WWWDownload_f( client_t *cl ) {
	...
	if ( !Q_stricmp( subcmd, "done" ) ) {
		...
		*cl->downloadName = 0;
		...
		cl->bFallback = qtrue;
		// send a reconnect
		Com_Printf( "SV_WWWDownload: client '%s' reports download failure.\n", cl->name );
		SV_SendClientGameState( cl );
		return;
...
//		Com_DPrintf( "Skipping flood protection for: %s\n", s );
	...
	if ( !com_cl_running->integer &&
		...
		// gaoesa, check if the clienst are disconnected for downloads before discarding based on the state
		//( (sv_wwwDlDisconnected->integer) || (cl->state >= CS_ACTIVE) ) &&      // (SA) this was commented out in Wolf.  Did we do that?
		floodprotect ) {
