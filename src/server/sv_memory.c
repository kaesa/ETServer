...
#endif //MEMDEBUG
{
	...
	ptr = Z_TagMalloc( size + sizeof( memoryblock_t ), TAG_BOTLIB );
...
#endif //MEMDEBUG
{
	...
	if ( Hunk_CheckMark() ) {
		Com_Error( ERR_DROP, "GetHunkMemory: Alloc with marks already set\n" );
	}
	ptr = Hunk_Alloc( size + sizeof( memoryblock_t ), h_high );
	block = (memoryblock_t *) ptr;
	block->id = HUNK_ID;
	block->ptr = (char *) ptr + sizeof( memoryblock_t );
	block->size = size + sizeof( memoryblock_t );
...
#endif //MEMDEBUG
{
	...
	ptr = Z_TagMalloc( size + sizeof( unsigned long int ), TAG_BOTLIB );
	if ( !ptr ) {
		return NULL;
	}
...
#endif //MEMDEBUG
{
	...
	ptr = Hunk_Alloc( size + sizeof( unsigned long int ), h_high );
	if ( !ptr ) {
		return NULL;
	}
	memid = (unsigned long int *) ptr;
