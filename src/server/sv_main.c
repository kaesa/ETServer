...
// OOB flooding
cvar_t	*sv_AF_OOBPackets;
cvar_t	*sv_AF_OOBSeconds;
cvar_t	*sv_AF_OOBRestTime;
	...
	for ( i = 0 ; i < MAX_MASTER_SERVERS ; i++ ) {
		...
		if ( sv_master[i]->modified ) {
			...
			if ( !NET_StringToAdr( sv_master[i]->string, &adr[i] ) ) {
				...
				//sv_master[i]->modified = qtrue; // uncomment this and comment the 2 following if always keep resolving
		...
		if( Cvar_Get( "sv_useFacade", "1", CVAR_LATCH)->integer ) {
			adr[i].type = NA_IPF;
			NET_OutOfBandPrint( NS_SERVER, adr[i], "heartbeat %s\n", hbname );
		}
	}
...
void SVC_Status( netadr_t from ) {
	...
	if( from.type != NA_IPF ) {
		...
		Info_SetValueForKey( infostring, "protocol", va( "%i", FACADE_PROTOCOL_VERSION ) );
	}
...
qboolean SV_OOBIsFloodPacket(msg_t *msg)
{
	if( msg->cursize < 10 ) {
		return qfalse;
	}
	if( !Q_stricmpn(&msg->data[7], "info", 4) ) {
		return qtrue;
	}
	// check we are checking only getstatus and getinfo packets
	if( msg->cursize < 13 ) {
		return qfalse;
	}
	if( !Q_stricmpn(&msg->data[7], "status", 6) ) {
		return qtrue;
	}
	return qfalse;
}
...
/**
	Function filters packets from http/https ports
*/
qboolean SV_OOBFlooding(netadr_t from, msg_t *msg)
{
	static int maxSize = 128;
	static ipTries_t* ddos = NULL; 
	static int ddosSize = 0;
	int i,timeNow;
	ipTries_t tempstruct;
	...
	if (ddos == NULL ) {
		ddos = (ipTries_t*)malloc(maxSize * sizeof(ipTries_t));
	}

	if( ddos == NULL ) {
		Com_Printf("[ANTI-FLOOD] Memory allocation failed. Passing all packets through.\n");
		return qfalse;
	}

	if( !SV_OOBIsFloodPacket(msg) ) {
		return qfalse;
	}
	...
	for(i = 0; i < ddosSize; i++) {
		if (from.ip[0] == ddos[i].ip.ip[0] || from.ip[1] == ddos[i].ip.ip[1] || from.ip[2] == ddos[i].ip.ip[2] || from.ip[3] == ddos[i].ip.ip[3]){
			...
			if( (ddos[i].someStatus == 1) && (timeNow - ddos[i].lastReq) > sv_AF_OOBRestTime->integer ){
				...
				ddos[i].tries = 1;
				ddos[i].someStatus = 0;
				Com_Printf("[ANTI-FLOOD] At %d, Address %d.%d.%d.%d is released and can flood again\n",timeNow, from.ip[0],from.ip[1],from.ip[2],from.ip[3]);

				return qfalse;
			}

			if( ddos[i].someStatus == 1 ) {
				// maintain blocking
				ddos[i].lastReq = timeNow;
				return qtrue;
			}

			if( (ddos[i].firstReq + sv_AF_OOBSeconds->integer) >= timeNow ) {
				ddos[i].tries++;
			} else {
				ddos[i].firstReq = ddos[i].lastReq;
				ddos[i].tries = 1;
			}
			ddos[i].lastReq = timeNow;

			if( ddos[i].tries > sv_AF_OOBPackets->integer ) {
				int port = BigShort(from.port);
				if(ddos[i].someStatus == 0) {
					Com_Printf("[ANTI-FLOOD] At %d, (limit %d/%d second) Packets from %d.%d.%d.%d:%d were flooding the server. Reached threshold rate in %d seconds.\n", timeNow, sv_AF_OOBPackets->integer, sv_AF_OOBSeconds->integer, from.ip[0],from.ip[1],from.ip[2],from.ip[3], port, (ddos[i].lastReq - ddos[i].firstReq));
					ddos[i].someStatus = 1;
				}
	...
	// check if we can reuse old entry for a new record
	for(i = 0; i < ddosSize; i++) {
		// reusing records if they havent sent OOB packets in full resting time
		if( (timeNow - ddos[i].lastReq) > (sv_AF_OOBRestTime->integer *2) ) {
			ddos[i].ip = from;
			ddos[i].tries = 1;
			ddos[i].someStatus = 0;
			ddos[i].firstReq = timeNow;
	...
	tempstruct.firstReq = timeNow;
	tempstruct.lastReq = timeNow;

	ddosSize++;
	if (ddosSize >= maxSize) {
		maxSize += 128;
		Com_Printf("[ANTI-FLOOD] Increasing my memory, thank you for your space (%d)!\n",maxSize);

		ddos = (ipTries_t*)realloc(ddos, maxSize* sizeof(ipTries_t));
...
void SV_PacketEvent( netadr_t from, msg_t *msg ) {
	...
	port = BigShort(from.port);

	// first filter out known bad addresses
	if( port == 53 ) {
		return;
	}
	...
	if ( msg->cursize >= 4 && *(int *)msg->data == -1 ) {
		// then flood filtering, internet services dont need fast
		if( SV_OOBFlooding(from, msg) ) {
			return;
		}
	...
	for ( i = 0, cl = svs.clients ; i < sv_maxclients->integer ; i++,cl++ ) {
		...
		if ( SV_Netchan_Process( cl, msg ) ) {
			...
			// gaoesa, testing to handle commands only for clients that have received gamestate
	...
	Com_Printf("Statefull packet from: %d.%d.%d.%d.\n", from.ip[0], from.ip[1], from.ip[2], from.ip[3]);
