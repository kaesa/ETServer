...
char    * QDECL va( char *format, ... );
	...
	// tjw: i'm disabling silEnT_dsnoinfo() because it depends on
	...
	void CrashHandler(int signal, siginfo_t *siginfo, ucontext_t *ctx) {
		...
		if(segvloop < 2) {
			...
			G_LogPrintf("   Please forward to " MOD_NAME_CLR " mod team.   \n");
			G_LogPrintf("---------------------------------------\n");
	...
	/*
		Visual C 7 Users, place the PDB file generated with the build into your silEnT 
	...
	*/
	...
	void win32_exceptioninfo(LPEXCEPTION_POINTERS e) {
		Com_Printf("Exception: %s (0x%08x)\n", ExceptionName(e->ExceptionRecord->ExceptionCode), e->ExceptionRecord->ExceptionCode); 
		Com_Printf("Exception Address: 0x%08x\n", e->ExceptionRecord->ExceptionAddress);
	}

	void win32_dllinfo() {
		Com_Printf("DLL Information:\n");
	...
	LONG CALLBACK win32_exception_handler(LPEXCEPTION_POINTERS e) {
		...
		gamepath = Cvar_VariableString("fs_game");
		pfnSymInitialize(GetCurrentProcess(), va("%s\\%s", basepath, gamepath), TRUE);
		Com_Printf("-8<------- Crash Information ------->8-\n");
		Com_Printf("---------------------------------------\n");
		Com_Printf("Version: %s %s Win32\n", "Server Project", "0.0.1");
	...
	void win32_initialize_handler(void) {
		...
		if(!imagehlp) {
			Com_Printf("imagehlp.dll unavailable\n");
			return;
		}
		...
		if(
			...
			Com_Printf("imagehlp.dll missing exports.\n");
			return;
		}
