...
static SOCKET ip_facade_socket;
...
void NetadrToSockadr( netadr_t *a, struct sockaddr *s ) {
	...
	} else if ( a->type == NA_IP || a->type == NA_IPF )    {
		( (struct sockaddr_in *)s )->sin_family = AF_INET;
...
qboolean Sys_GetPacket( netadr_t *net_from, msg_t *net_message ) {
	...
	for ( protocol = 0 ; protocol < 2 ; protocol++ ) {
		...
		if ( net_socket == ip_socket || net_socket == ip_facade_socket ) {
			memset( ( (struct sockaddr_in *)&from )->sin_zero, 0, 8 );
		}

		if ( usingSocks && (net_socket == ip_socket || net_socket == ip_facade_socket) && memcmp( &from, &socksRelayAddr, fromlen ) == 0 ) {
			if ( ret < 10 || net_message->data[0] != 0 || net_message->data[1] != 0 || net_message->data[2] != 0 || net_message->data[3] != 1 ) {
				continue;
			}
			if( net_socket== ip_facade_socket ) {
				net_from->type = NA_IPF; // fixing the from after it is set normally first
			} else {
				net_from->type = NA_IP;
			}
...
void Sys_SendPacket( int length, const void *data, netadr_t to ) {
	...
	if ( usingSocks && (to.type == NA_IP || to.type == NA_IPF)) {
	...
	if ( ret == SOCKET_ERROR ) {
		...
		Com_Printf( "NET_SendPacket: %s in %s\n", NET_ErrorString(), (to.type == NA_IPF ? "facade port" : "primary port"));
	}
}
...
qboolean Sys_IsLANAddress( netadr_t adr ) {
	...
	if ( adr.type != NA_IP && adr.type != NA_IPF ) {
		return qfalse;
	}
...
void NET_OpenIP( void ) {
	...
	if( Cvar_Get( "sv_useFacade", "1", CVAR_LATCH)->integer ) {
		facade_port = Cvar_Get( "sv_facadePort", va( "%i", PORT_SERVER + 1), CVAR_LATCH )->integer;
	} else {
		facade_port = 0;
	}
	...
	for ( i = 0 ; i < 10 ; i++ ) {
		...
		if( facade_port ) {
			ip_facade_socket = NET_IPSocket( ip->string, facade_port + i );
		}

		if( ip_facade_socket ) {
			Cvar_SetValue( "sv_facadePort", facade_port + i );
			return;
		}
	}
