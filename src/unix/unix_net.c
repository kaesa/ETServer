...
void    Sys_SendPacket( int length, const void *data, netadr_t to ) {
	...
	if ( ret == -1 ) {
		Com_Printf( "NET_SendPacket ERROR: %s to %s in %s\n", NET_ErrorString(), NET_AdrToString( to ) , (to.type == NA_IPF ? "facade port" : "primary port"));
	}
}
